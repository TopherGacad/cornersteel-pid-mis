<?php
    ...
    //----------------------------------------------------------------------------//
    //           DOCUMENT HANDLING ALL THE SCRIPT FOR THE EMPLOYEES (EDIT)         //
    //----------------------------------------------------------------------------//
    ...
    if(isset($_POST['emp-update'])){
        ...
        EditEmployee($conn, $empid, $company, $department, $firstname, $middlename, $lastname, $age, $phone, $position, $email, $sex);
    }
    else{
        //--- TAKES USER BACK TO THE MAIN PAGE 'main.php' WHENEVER DATA INSERTION FAILS ---//
        header("Location: ../../frontend/views/php/main.php?Empupdate=failed");
        exit();
    }
